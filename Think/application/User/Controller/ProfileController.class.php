...
class ProfileController extends MemberbaseController {
    ...
    public function edit_post() {
    	...
    	if(IS_POST){
			...
			$temp = array(" userphone = '$userphone' ");
			array_push($temp," user_email = '$user_email' ");
			$temp1=join("or", $temp);
						
			$user1 = $users->where($temp1)->find();
		
			
			if(!empty($user1) and $user1['id']!=$userid){
				$this->error("邮箱或手机号已存在");
			}else{
				
				if ($this->users_model->create()) {
				if ($this->users_model->save($where)!==false) {
					
					
					$user=$this->users_model->find($userid);
					sp_update_current_user($user);
					$phone['userphone']=$userphone;
					$result = $users->where($check)->save($phone);
