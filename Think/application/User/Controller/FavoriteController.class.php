...
class FavoriteController extends MemberbaseController{
	...
	function index(){
		...
		$user_favorites_model=M("user_favorites");
		$count=$user_favorites_model->where("uid=$uid")->count();
		$favorites=$user_favorites_model->where("uid=$uid")
	...
	function reservate(){
		...
		$reser = $reservation->where("userid=$userid")->order("reserid DESC")->select();
		for( $i=0;$i<count($reser);$i++){			
			$reser[$i]['reservate_time']=date('Y-m-d',strtotime($reser[$i]['reservate_time']));
		}
		
		$this->assign("reservation",$reser);
		$this->display(":myreservation");
