...
class RegisterController extends HomeBaseController {
	...
	if(eregi("^[0-9]+$",$username)){
                    $this->error('用户名不能全为数字！');
	}

            if( !(eregi("^[0-9]+$",$userphone)) ){
                    $this->error('手机号应该全为数字！');
          }
    	...
    	if(strlen($password) < 5 || strlen($password) > 20){
    		$this->error("密码长度至少5位，最多20位！");
             ...
             if(preg_match('/['.$stripChar.']/is', $password)==1){
                    $this->error('密码中包含'.$stripChar.'等非法字符！');
             }
    	...
    	if($ucenter_syn){
			...
			$uc_checkphone=uc_user_checkphone($userphone);
    	}
    	
    	$result = $users_model->where($where)->count();
    	if($result || $uc_checkphone<0 ||$uc_checkemail<0 || $uc_checkusername<0){
    		$this->error("用户名或该邮箱或手机号已经存在！");
    	}else{
    		$uc_register=true;
    		if($ucenter_syn){
    	
    			$uc_uid=uc_user_register($username,$password,$email,$userphone);
    			//exit($uc_uid);
    			if($uc_uid<0){
    				$uc_register=false;
    			}
